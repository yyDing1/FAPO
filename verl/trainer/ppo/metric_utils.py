...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> Dict[str, Any]:
    ...
    if "false_positive" in batch.non_tensor_batch["reward_extra_infos"][0]:
        sequence_acc = [info["acc"] for info in batch.non_tensor_batch["reward_extra_infos"]]
        sequence_false_positive = [info["false_positive"] for info in batch.non_tensor_batch["reward_extra_infos"]]
        acc = np.mean(sequence_acc)
        false_positive = np.mean(sequence_false_positive)
        false_positive_ratio = false_positive / (acc + 1e-5)
        metrics["fp_exp/acc"] = acc
        metrics["fp_exp/false_positive"] = false_positive
        metrics["fp_exp/false_positive_ratio"] = false_positive_ratio
...
def calc_maj_val(data: list[dict[str, Any]], vote_key: str, val_key: str, invalid_str: str = "[INVALID]") -> float:
    """
            ...
            for var_name, var_vals in var2vals.items():
                ...
                if n_resps > 1 and var_name == "acc":
                    ...
                    for n in ns:
                        ...
                        metric[f"best@{n}"] = bon_mean
                        # metric[f"worst@{n}/mean"], metric[f"worst@{n}/std"] = won_mean, won_std
                        metric[f"worst@{n}"] = won_mean

...
class RayPPOTrainer:
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            non_tensor_batch_keys_to_pop = [
                key for key in non_tensor_batch_keys_to_pop if key in test_batch.non_tensor_batch
            ]
            non_tensor_batch_keys_to_add = ["data_source", "reward_model", "extra_info"]
            non_tensor_batch_keys_to_add = [
                key for key in non_tensor_batch_keys_to_add if key in test_batch.non_tensor_batch
            ]
            ...
            test_gen_batch_add = test_batch.select(
                non_tensor_batch_keys=non_tensor_batch_keys_to_add,
            )
            test_gen_batch.union(test_gen_batch_add)
            ...
            if "reward_tensor" in test_batch.batch:
                reward_tensor = test_batch.batch["reward_tensor"]
                reward_extra_infos = test_batch.non_tensor_batch.get("reward_extra_infos", None)
        ...
        metric_dict = {}
        # For ProcessBench
        if data_sources[0] == "ProcessBench":
            assert "pred" in reward_extra_infos_dict
            assert "gt" in reward_extra_infos_dict
            pred_p, true_fp, pred_true_p = 0, 0, 0
            for pred, gt in zip(
                reward_extra_infos_dict["pred"],
                reward_extra_infos_dict["gt"],
            ):
                if gt != -1:  # false positive
                    true_fp += 1
                if pred is not None and pred != -1:
                    pred_p += 1
                if gt != -1 and pred is not None and pred != -1:
                    pred_true_p += 1
            recall = pred_true_p / true_fp
            precision = pred_true_p / pred_p
            f1 = 2 * recall * precision / (recall + precision)
            metric_dict["val-core/ProcessBench/recall"] = recall
            metric_dict["val-core/ProcessBench/precision"] = precision
            metric_dict["val-core/ProcessBench/f1"] = f1
            metric_dict["val-core/ProcessBench/acc"] = np.mean(reward_extra_infos_dict["acc"])
        else:
    ...
    def init_workers(self):
        ...
        if self.config.actor_rollout_ref.rollout.mode == "async":
            from verl.experimental.agent_reward_loop import AgentRewardLoopManager

            self.async_rollout_mode = True
            self.async_rollout_manager = AgentRewardLoopManager(
                config=self.config,
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                non_tensor_batch_keys_to_pop = [
                    key for key in non_tensor_batch_keys_to_pop if key in batch.non_tensor_batch
                ]
                ...
                if self.async_rollout_mode:
                    non_tensor_batch_keys_to_add = ["data_source", "reward_model", "extra_info"]
                    non_tensor_batch_keys_to_add = [
                        key for key in non_tensor_batch_keys_to_add if key in batch.non_tensor_batch
                    ]
                    gen_batch_add = batch.select(
                        non_tensor_batch_keys=non_tensor_batch_keys_to_add,
                    )
                    gen_batch.union(gen_batch_add)
                ...
                with marked_timer("step", timing_raw):
                    ...
                    if "reward_tensor" not in batch.batch:
                    ...
                    with marked_timer("adv", timing_raw, color="brown"):
                        ...
                        if "reward_tensor" in batch.batch:
